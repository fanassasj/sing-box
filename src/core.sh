...
info() {
    ...
    case "$net" in # $net is crucial here, set by `get protocol` which is called by `get info`
        ws | tcp | h2 | quic | http*) # Covers TLS and non-TLS with these transports
            if [[ "$host" ]]; then # TLS variants with host
                ...
                [[ "$is_protocol" == 'vmess' ]] && {
                    ...
                    [[ "$is_protocol" == "trojan" ]] && { uuid="$password"; is_can_change=(0 1 2 3 4); is_info_show=(0 1 2 10 4 6 7 8); }
                ...
                if [[ "$net" == "http" ]]; then is_type=http; is_tcp_http=1; is_info_show+=(5); is_info_str=("${is_info_str[@]/http/tcp http}"); fi
                [[ "$net" == "quic" ]] && { is_insecure=1; is_info_show+=(8 9 20); is_info_str+=('tls' 'h3' 'true'); is_quic_add=",tls:\"tls\",alpn:\"h3\""; }
        ...
        socks) is_can_change=(0 1 12 4 15 16); is_info_show=(0 1 2 19 10); is_info_str=("$is_protocol" "$is_addr" "$port" "$is_socks_user" "$is_socks_pass"); is_url="socks://$(echo -n "${is_socks_user}:${is_socks_pass}" | base64 -w 0)@${is_addr}:${port}#${display_protocol_name}-${is_addr}"; [[ "$socks_allow_enable" == 1 ]] && msg "Socks白名单已启用: $socks_allow_list" ;;    
        http) is_can_change=(0 1 13 14 17 18); is_info_show=(0 1 2 19 10); is_info_str=("$is_protocol" "$is_addr" "$port" "$http_user" "$http_pass"); is_url="http://$(echo -n "${http_user}:${http_pass}" | base64 -w 0)@${is_addr}:${port}#${display_protocol_name}-${is_addr}"; [[ "$http_allow_enable" == 1 ]] && msg "HTTP白名单已启用: $http_allow_list" ;;    
        mtproto) is_can_change=(0 1 19 20 21); is_info_show=(0 1 2 22); is_info_str=("$is_protocol" "$is_addr" "$port" "$mtp_secret"); is_url="tg://proxy?server=$is_addr&port=$port&secret=$mtp_secret"; [[ "$mtp_allow_enable" == 1 ]] && msg "MTP白名单已启用: $mtp_allow_list" ;;    
    esac
